...
3. [x] При git pull меняются ли локальные ветки
...
>[!question]- чем отличается fetch от pull 
> fetch не изменяет ветку при получении удаленных данных , а  pull меняет через merge / rebase
> git pull = git fetch + слияние в ТЕКУЩУЮ ветку.  при этом остальные локальные ветки не сливаются с удаленными
> Чтобы влить удаленные изменени я в локальную ветку можно
> 1. git checkout A && git pull (не проверено)
> 2. git fetch origin A:A B:B (не проверено)

>[!question]- Удалить ветку безопасно
>  git checkout develop          # перейти в ту ветку, куда фича вливалась
>  git pull                      # подтянуть свежее состояние
>   git log develop..feature      # что есть в feature, но НЕТ в develop
>   Если последняя команда вывела пусто — всё содержимое ветки уже в develop, удалять безопасно:
>   git branch -d feature                # локально    
>   git push origin --delete feature     # на сервере
>   - git branch -d — безопасный. Git откажется удалять, если в ветке есть коммиты, которых нет в текущей ветке (или в её upstream). Это твоя страховка. 
>   - git branch -D — принудительный. Сносит что угодно. Использовать, только когда ты сам проверил, что терять нечего.     
...
>git -c credential.helper= -c core.quotepath=false -c log.showSignature=false merge --abort 
  error: Entry 'backend-java/src/main/resources/application.properties' not uptodate. Cannot merge.                                    
  fatal: Could not reset index file to revision 'HEAD'. 
>```
>Эта ошибка означает, что файл application.properties имеет несохраненные изменения, и Git не может отменить merge, чтобы не потерять эти изменения.     
> На сильно откатить это файл и потом выполнить  ?
> ```
...
>[!question]- Переименовать сообщение последнего коммита
>В терминале:
>```
>git commit --amend -m "Новое сообщение коммита"
>```
>Или без флага `-m` — откроется редактор:
>```
>git commit --amend
>```
...
>[!question]- Отличие **rebase** от merge 
>**merge**  сохраняет коммиты локальной ветки разработчика - используется для публичной истории , а
>**rebase** (+ git push force) меняет их меняет -  "**перебазирует"** точку ветвления - во всех коммитах рабочей ветки появляются коммиты develop и искажается история изменений ветки (коммитов). Стирается только топология ветвления.
>Без git push force придется вибирать merge / rebase . 
>rebase — удобство читателя истории, оплаченное автором ветки.
>merge — безопасность совместной работы, оплаченная шумом в истории.  
...
# **Git stash** 
==команда, которая позволяет временно сохранить («припрятать») ваши текущие незавершенные изменения, чтобы очистить рабочую директорию и вернуться к чистому состоянию ветки==.

Это удобно, когда нужно срочно переключиться на другую задачу (например, пофиксить баг в другой ветке), но вы еще не готовы делать коммит. 
Основные моменты:

- **Где хранятся изменения:** В специальном локальном хранилище (стеке), который не передается на сервер при `push`.
- **Что сохраняется:** Изменения в отслеживаемых файлах (индексированных и нет). Новые (untracked) файлы по умолчанию не попадают в stash, если не добавить флаг `-u`.
- **Принцип работы:** Используется принцип **LIFO** (последним пришел — первым ушел).

Базовые команды:

- `git stash` — сохранить изменения и вернуть рабочую копию к состоянию последнего коммита.
- `git stash list` — посмотреть список всех сохраненных «заначек».
- `git stash pop` — вернуть последние изменения и **удалить** их из хранилища.
- `git stash apply` — вернуть изменения, но **оставить** их в хранилище.
- `git stash drop` — удалить конкретную запись из списка.
- `git stash clear` — полностью очистить всё хранилище.
