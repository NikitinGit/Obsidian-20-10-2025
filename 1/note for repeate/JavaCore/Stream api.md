...
5. [ ] у разных коллекций разные стримы? (Set, HashMap, TreeMap...)
6. [ ] существует ли общее правило в reversed в sort
7. [ ] отличие Collectors.toMap от Collectors.groupingBy
8. [ ] паралельный стрим, что в какой поток попадает
9. [x] задачи по стримам 
10. [x] ```flatMap``` попробуй со списком объектов, внутри каждого из которых есть список или массив
11. [ ] ```flatMap``` что может быть в качестве параметра, типизация и перевод одного типа в другой
12. [x] Collectors
