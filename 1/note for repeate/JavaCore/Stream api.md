...
6. [x]  почему у некоторых функциональных интерфейсов в jdk 2 метода
7. [ ] когда parallelStream медленнее чем stream
8. [ ] у разных коллекций разные стримы? (Set, HashMap, TreeMap...)
9. [ ] отличие типовых стримов (IntStream, DoubleStream ....) от stream
10. [ ] map vs mapObject (``mapToDouble() возвращает DoubleStream — это примитивный поток, не Stream<Double>.``)
11. [ ] существует ли общее правило в reversed в sort
12. [x] отличие Collectors.toMap от Collectors.groupingBy
13. [ ] паралельный стрим, что в какой поток попадает
14. [ ]  ```flatMap``` - как равзернуть 3 и более вложенных уровня
15. [x] ```flatMap``` попробуй со списком объектов, внутри каждого из которых есть список или массив
16. [x] ```flatMap``` что может быть в качестве параметра, типизация и перевод одного типа в другой, с тремя и более вложенностями
17. [x] Collectors
