...
11. [ ] основная проблема многопоточки - race conditiion - потерянное обновление - как решается - что видно что нет 
12. [ ] Unboxing при  + - / *  в стримах напиши
13. [ ] Supplier - поставщик 
14. [ ] Зачем нежен hashCode() и как он создается, может ли он измениться -ДОПИШИ
15. [ ] HashMap -   в каком порядке вставляются элементы
16. [ ] TreeMap - как работает reverseOrder()
17. [x] instanceof зачем пишут в equals переопределиние метода 
18. [x] Методы object - посмотри в коде 12.55 -14.37
19. [x] иквелс (у всех насдеыдников одина ссылка на объект )
20. [x] тостринг
21. [x] instanceof что делает 
22. [ ] hashCode
23. [ ] clone
24. [ ] getClass
25. [ ] notify
26. [ ] notifyAll
27. [ ] wait
28. [ ] дженерики - ковариантность, инвариантность , контвариантность 
29. [ ] Методы рефлексии
30. [ ] как Thread.currentThread() связан с Object
31. [ ] мониторинг программ 
32. [ ] Посмотри лямбда в стрим апи линекд хеш мап 
33. [ ] Почему при создании хешмап сбивается сортировка   

# BASE

>[!question]- Stateless vs Stateful — что это и откуда термин
>**CS (Computer Science)** — это академическое название информатики как науки. Термин "из CS" значит: понятие придумано не в Java и не в стримах — оно общее для всего программирования, просто Java его использует.
>
>**Stateful** (с состоянием) — объект/операция **помнит** предыдущие данные, её результат зависит от истории.
>**Stateless** (без состояния) — каждый вызов независим, результат зависит только от текущего входа.
>
>**В Stream API:**
>- Stateless операции (`filter`, `map`, `flatMap`) — не помнят предыдущие элементы → работают в конвейере, элемент за элементом
>- Stateful операции (`sorted`, `distinct`) — обязаны видеть ВСЕ элементы прежде чем выдать хоть один → барьер, прерывают конвейер
>
>**Тот же термин в других местах:**
>- HTTP — stateless протокол: каждый запрос независим, сервер не помнит предыдущий
>- REST API — должен быть stateless: не хранить сессию между запросами
>- Сессия в браузере — stateful: сервер помнит кто ты
>- Переменная в лямбде — effectively final, потому что лямбда stateless по контракту
>- Микросервисы — стремятся к stateless для горизонтального масштабирования
...
>  for (int c = ctl.get(); ; ) -
>```
>В for может отсутствовать init если он объявлен до него
>```
>int n = 0;
>for (; ; ) {
>if (n >= 3) break;
