...
17. [x] место амкп и тсп в модели сети ОСИ
18. [x] как выглядят сохранненные сообщения в реббите при дурабилити 
19. [x] как устроен амкп фрейм 
20. [x] отличие структуры амкп фрейма от джсон хттп запроса 12/50 - 13/18
21. [ ] Correlation Id это , для чего используется 
22. [ ] зачем нужен @Bean  MessageListenerAdapter если есть @RabbitListener 
23. [ ] что делает каждый бин в конфиг классе 
24. [x] RPC относится ко всем брокерам сообщений или только к раббиту 13/10 -  14/41
25. [ ] чем рпс отличается от закидывания сообщения в очередь - только тем что во втором нет бизнеслогики или тем что кроме сообщения в виде назавания функции / метода  передаются его параметры? 
26. [ ] как удалить очередь и обменник
27. [ ] что в моем тестовом проекте рпс 
28. [ ] как через рест тамплате или рабиттамплет опарвлять запросы с джсон телом, посмотри в своем проекте с оплатой , нужно ли конфиг настраивать для этого или это не обязательно
29. [ ] допиши понятия полученный из аи ассистента гугл в поиске 
30. [ ] когда  конфигурация обменника и очереди должны совпадать на консьюмере и продюссере
31. [x] как заменить метод слушатель на @RabbitListener(queues = "rpc-order-queue") это возможно ? - да
32. [x] Если топик при разных очередях работает так же как фанаут  то в том отличие 
33. [x] типы обменников (директ, топик , фанаут) - напиши 3 роута - проверь послещний директ 
34. [x] fanout **Веерная рассылка** 
35. [ ] шаблон Pub/Sub (публикация/подписка) - поведенческий паттерн
36. [ ] воркер и инстанс - в чем разница
37. [ ] "Место брокеров сообщений в систем дизайне"
...
>[!question]- AMQP
>Advanced Message Queuing Protocol - Работает поверх TCP/IP  (TCP передает  AMQP фреймы) - прикладной протокол - 7 уровень ОСИ (прикалдной - application)
...
>  4. Брокеры использующие этот протокол 
> ```
...
>  IBM MQ 
>``` 
> Брокеры  могут использовать разные версии AMQP и поэтому структура фрейма может отличаться - взаимодействовать могут только брокеры с одинаковый версией AMQP
> 5. AMQP   фрейм - это структурированный пакет данных, который Spring AMQP автоматически создает когда вы вызываете:  
>  ```
>  messageSenderService.sendMessage(
>  "foo.bar.baz",
  >"Topic Nikitin Сообщение 1",
>  "spring-boot-exchange"
>);
>  ``` 
>  Под капотом Spring AMQP создает 3 AMQP frames:  
>  Frame 1: Method Frame (ЧТО делать? например "Publish" (отправить сообщение) ,  Куда: Exchange + Routing Key ,  Как: Mandatory? Immediate? ) , 
>  Frame 2: Header Frame  СВОЙСТВА сообщения , Метаданные: Content-Type, Encoding , Доставка: Durable? Priority? ,  RPC: Reply-To, Correlation-ID 
>  Frame 3: Body Frame (само сообщение)
>  6. Преимущества деления сообщения на 3 фрейма:
  ✅ RabbitMQ сразу видит метаданные (Frame 2) до получения тела
  ✅ Может начать routing ДО получения всего файла
  ✅ Может отклонить файл (по размеру) до скачивания
  ✅ Streaming - файл передается по частям
...
>7. Асинхронность - отправитель не должен ждать ответа от получателя как в рест запросах - не блокирует апи дорогими опреациями 
>8. Отказаустойчивость - сообщения хранятся в очереди 
>9.  По умолчанию RabbitMQ НЕ гарантирует доставку! Нужна явная настройка всех трех уровней. ( Publisher Confirms   Продюсер ← Брокер - Гарантирует, что сообщение дошло до брокера и сохранено в exchange/queue ,  Message Persistence
...
> 10.  У одного подключения может быть несколько каналов  , для каждого потока свой канал иначе будет не безопасно , через один канал может передаватснься несколько сообщений 
...
>[!question]- Отличие  AMQP фреймов от  HTTP JSON
> HTTP JSON = Текстовый протокол (человекочитаемый) 
> AMQP Frame = Бинарный протокол (машинно-оптимизированный) 
> ```
>  Характеристика | HTTP JSON                 | AMQP Frame             |
  |----------------|---------------------------|------------------------|
  | Формат         | Текст ("Host: localhost") | Байты (0x01 0x00 0x01) |
  | Размер         | ~700 байт                 | ~360 байт (2x меньше)  |
  | Скорость       | Медленнее                 | Быстрее (2x)           |
  | Читаемость     | ✅ Можно прочитать        | ❌ Нужен декодер       |
  | Overhead       | 65%                       | 32%                    |
  | Debug          | ✅ Легко (Postman)        | ❌ Wireshark           |
> ```
> Наглядное сравнение:
> ```
...
> ```
> Frame 1 (Method):
  01 00 01 00 00 00 2D 00 3C 00 28 ...
  "rpc-exchange" "rpc.order.calculate"
  Frame 2 (Header):
  02 00 01 00 00 00 38 00 3C ...
  content-type="application/json"
  Frame 3 (Body):
  03 00 01 00 00 00 F5 7B 22 6F 72 ...
  {"orderId":"ORD-12345","items":[...]}
  Размер: ~360 байт (в 2 раза меньше!)
  Формат: Байты (нужен декодер ❌)
> ```
