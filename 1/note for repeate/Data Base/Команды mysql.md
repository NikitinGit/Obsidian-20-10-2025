# MYSQL
1. [ ] что такое SELECT 1 FROM и может  ли использоваться без EXISTS, почему EXISTS устраняет дубли
2. [ ] Типы индексов - индекс по условию,  JIN индекс для поля JSONB , индекс векторного поля и что это,
3. [ ] Explain и Analyze в Mysql(или аналоги хз чо там)
4. [ ] Как удалить таблицу если в ней есть связи
5. [ ] Отчистить таблицу 
6. [ ] Что такое UNION (объединение запросов в один) ALL и его типы 
7. [x] какой алгоритм используется в индексах 
