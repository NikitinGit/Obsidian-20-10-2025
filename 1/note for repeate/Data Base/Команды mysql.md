# MYSQL
1. [x] что такое SELECT 1 FROM и может  ли использоваться без EXISTS, почему EXISTS устраняет дубли 
2. [ ] IN / EXIST ПРОВЕРЬ РАЗНИЦУ
3. [ ] Типы индексов - индекс по условию,  JIN индекс для поля JSONB , индекс векторного поля и что это,
4. [ ] Explain и Analyze в Mysql(или аналоги хз чо там)
5. [ ] Как удалить таблицу если в ней есть связи
6. [ ] Отчистить таблицу 
7. [ ] Что такое UNION (объединение запросов в один) ALL и его типы 
8. [x] какой алгоритм используется в индексах 
...
>[!question]- SELECT 1 
>1. SELECT 1 (совсем без FROM) — вернёт ровно одну строку со значением 1, независимо ни от чего. Это и есть классический health-check пула соединений. А SELECT 1 FROM t — уже по строке на каждую запись таблицы t.
>2. В IN-подзапросе (тоже проверка существования, как EXISTS, но синтаксис другой):
>   ```
>  SELECT * FROM events e
>  WHERE e.event_id IN (SELECT event_id FROM events_bids_fighters WHERE approved = 1); 
>  -- тут в подзапросе НЕ 1, а колонка — потому что IN сравнивает значения  
>   ```
>   Важно: в IN SELECT 1 не подойдёт — там нужна реальная колонка для сравнения. SELECT 1 уместен именно в EXISTS (где значения игнорируются) и в самостоятельных проверках.
>3.  UNION для добавления «искусственной» строки:  
>   ```
>   SELECT name FROM events WHERE event_id = 175   
>   UNION ALL
>   SELECT 1;             -- добавит строку со значением '1'    
>   ```   

>[!question]- EXIST (SELECT 1 )
> "1" - это фиктивная константа  выполняющая роль заглужки
> ```
> SELECT 1 ... = SELECT 2 ... = SELECT *
> ```
> EXISTS работает так же, как LEFT JOIN без FETCH - после него требуется отдельный дозапрос связям внутри сущности

>[!question]- "?" в WHERE это 
>prepared statement - плейсхолдер / заглужка / подготовка запроса - 
>4. Защита от SQL-инъекций (главное) 
>```
>  PreparedStatement ps = conn.prepareStatement("select ... from events_bids_fighters where event_id = ?");
>  ps.setInt(1, 5);        // ← подставляем 5 в первый ? (нумерация с 1)
>  ps.executeQuery();      // БД выполняет с event_id = 5   
>```
>5. Производительность (переиспользование плана). БД парсит и строит план для шаблона ... where event_id = ? один раз, а потом выполняет его с разными значениями без повторного парсинга. При склейке каждый запрос  (event_id = 5, event_id = 6, ...) — новый текст → новый парсинг → новый план (нагрузка + распухание кеша планов).  
>6. Корректность типов/экранирования. JDBC сам правильно оформит дату, строку с кавычками, null и т.п. — не надо вручную экранировать.   
>Как увидеть реальные значения вместо ? (для отладки) — включить логирование bind-параметров:          
>```
>logging.level.org.hibernate.orm.jdbc.bind=TRACE  
>```
>PreparedStatement в Java — это просто обёртка над этим же механизмом БД: prepareStatement("... ?") → делает PREPARE, а ps.setInt(1, 5) + executeQuery() → делает EXECUTE ... USING. То есть ?  живёт в самой БД, JDBC лишь даёт к нему API. 
>```
>PREPARE stmt FROM 'SELECT * FROM events_bids_fighters WHERE event_id = ? AND approved = ?';
>-- 2. задаём значения в переменные 
>SET @eid = 5; 
>SET @ok = 0;
>-- 3. выполняем шаблон с этими значениями (? заполняются по порядку)
>EXECUTE stmt USING @eid, @ok;
>```
...
>7. Сообщить админу препрода — проверить `SHOW GLOBAL STATUS LIKE 'Aborted_connects'`, `max_connections`, логи MySQL.
>8. Обходить ретраями — скрипт `~/dumps/strikerstat/dump_preprod_retry.sh`, который гоняет `mysqldump` до 10 раз с паузой 5 сек, пока не получит дамп без ошибок в логе:
>```
>#!/usr/bin/env bash
>set -uo pipefail
>
