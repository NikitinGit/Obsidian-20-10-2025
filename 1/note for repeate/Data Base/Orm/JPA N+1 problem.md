...
1. [ ] Если вообще не исопльзовать в @Entity связи - то проблема N+1 снимется ? проверь выборку со связями и без 
2. [ ] если при дистинкт выбирать список сущностей и в ентитит переопределить иквелс по выбранным полям - то так можно настраивать уникальные сущности в выборке - проверь и спроси у ии
3. [ ] проверь **`LEFT JOIN FETCH`** по числу подзапросов - сравни с LEFT JOIN - есть ли отличие в скл запросах 
4. [ ] @EntityGraph сравни с **`LEFT JOIN FETCH`** 
5. [ ] проверь batchSize при выборке
6. [ ] проверь batchSize при вставке 
7. [ ] при каких хайбиренейт операциях происходит препеарстейтмент
8. [ ] всегда ли есть смысл делать связь на уровне Entity, а не только в БД (см. также [[Spring Base]])
**ПРОВЕРЬ**                                           
  9. Пагинация ломается: setMaxResults/Pageable вместе с fetch коллекции → Hibernate предупреждает HHH000104: firstResult/maxResults specified with collection fetch; applying in memory — грузит ВСЁ и режет страницу в                                                                                            
     памяти (опасно на больших данных). Для пагинации: @EntityGraph + пагинация по id, или two-query подход.                                                                                                                                                                                                        
  10. Нельзя fetch-ить ДВЕ коллекции сразу (MultipleBagFetchException для List) — только одну на запрос. Обход: Set вместо List, @EntityGraph, или отдельные запросы.                                                                                                                                                
  11. LEFT vs INNER: LEFT JOIN FETCH оставит события без ставок (с пустой коллекцией); просто JOIN FETCH (inner) их выкинет.   
...
>
> **Про `DISTINCT` здесь:** дубли корня создаёт только fetch коллекции `eventBidFighters` (`bf.fighter` — `@ManyToOne`, не размножает). На **Hibernate 5** без `DISTINCT` было бы 399 дублей `Event` → а т.к. возврат `Optional<Event>`, ещё и `NonUniqueResultException`; поэтому `distinct` был **обязателен**. На **Hibernate 6 (6.5) `distinct` НЕ нужен** — дубли корня убираются автоматически, `Optional` корректен и без него (проверено: тот же fetch без `distinct` даёт `size()=1`). Оставлен как legacy; можно убрать. Детали — в callout «Зачем `DISTINCT`…» ниже.
...
>[!question]- Зачем `DISTINCT` в JPQL с JOIN FETCH коллекции (и почему в Hibernate 6 он НЕ нужен)
> JOIN с коллекцией размножает строки родительской сущности: если у event'а 399 bid'ов, SQL вернёт 399 строк, каждая с тем же event'ом.
>
> **Hibernate 5:** без `DISTINCT` Hibernate соберёт 399 ссылок на один и тот же Java-объект в результат → `DISTINCT` был **обязателен**. При этом был флаг `hibernate.query.passDistinctThrough=false`, чтобы `DISTINCT` НЕ уходил в SQL (на широкой выборке `SELECT DISTINCT` — лишняя нагрузка на БД), а дедуп шёл в памяти.
>
> **Hibernate 6 (проверено на 6.5):** дубли корня убираются **АВТОМАТИЧЕСКИ** — `DISTINCT` для дедупликации **не нужен**. Проверка: один и тот же `JOIN FETCH` без `distinct` и с `distinct` дают одинаковый `list.size()=1` (у event'а 399 bid'ов, коллекция в обоих случаях полная — 399). Поведение по сравнению с H5 **перевернулось**:
> - без `distinct` → корень дедуплицируется сам (в H5 было бы 399 дублей);
> - написать `distinct` теперь → Hibernate **пропускает** его в SQL (`select distinct` виден в логе), что для fetch join коллекции **бесполезно** (строка уникальна по PK ребёнка) и лишь добавляет нагрузку.
>
> **Вывод:** на Hibernate 6 `distinct` в `JOIN FETCH` коллекции **не пишем** — не нужен и слегка вреден. `distinct` остаётся нужным для других случаев: проекции/скаляры (`SELECT DISTINCT e.city`), агрегаты (`COUNT(DISTINCT …)`), и, возможно, обычный (не-fetch) `JOIN`, возвращающий сущности (авто-дедуп — только для fetch join).
