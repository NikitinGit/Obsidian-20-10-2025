...
2. [ ] проверь **`LEFT JOIN FETCH`** по числу подзапросов - сравни с LEFT JOIN - есть ли отличие в скл запросах 
3. [ ] @EntityGraph сравни с **`LEFT JOIN FETCH`** 
4. [ ] проверь batchSize при выборке
5. [ ] проверь batchSize при вставке 
6. [ ] при каких хайбиренейт операциях происходит препеарстейтмент
7. [ ] всегда ли есть смысл делать связь на уровне Entity, а не только в БД (см. также [[Spring Base]])
**ПРОВЕРЬ** 
  8. DISTINCT обязателен при fetch коллекции — иначе корневая сущность дублируется по числу детей (по строке на ребёнка).                                                                                                  +1 file edited before this session (show)                                                
