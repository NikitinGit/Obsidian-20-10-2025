...
>[!question]- как сделать так чтобы он перезапускался после рестатрта компа
>```
>nginx:
...
>restart: always
>```
> Варианты restart policy:
  >| Значение       | Описание                                                            |
  >|----------------|---------------------------------------------------------------------|
  >| no             | Не перезапускать (по умолчанию)                                     |
  >| always         | Всегда перезапускать, даже если остановлен вручную                  |
  >| unless-stopped | Всегда перезапускать, кроме остановленных вручную ⭐ (рекомендуется) |
