# NGINX
1. [ ] как устроен location и под каждый ли ендпоинт надо его писать 
2. [ ] что такое proxy_pass 
...
>[!question]- nginx может читать/отправлять пакеты на любом протоколе?
> Нет, есть чёткая граница между тем, что nginx **понимает** (разбирает содержимое), и тем, что просто **слепо перекачивает**.
>
> **1. Протоколы, которые nginx реально понимает (парсит содержимое):**
> - **HTTP-семейство** (`http{}`) — HTTP/1.x, HTTP/2, WebSocket (через `Upgrade`), в некоторых сборках экспериментально HTTP/3/QUIC. Здесь nginx реально разбирает заголовки, статусы, маршрутизирует по `location`/URL, переписывает заголовки.
> - **FastCGI / uwsgi / SCGI** (`fastcgi_pass`, `uwsgi_pass`, `scgi_pass`) — протоколы для PHP-FPM, Python-приложений и т.д.
> - **Почтовые протоколы** (`ngx_mail_*`) — SMTP, IMAP, POP3, но только на уровне прокси-аутентификации.
> - **memcached** (`memcached_pass`).
>
> **2. `stream{}` (L4, TCP/UDP) — слепой байтовый релей.** Здесь nginx **не понимает содержимое вообще**, просто перекачивает сырые байты между клиентом и апстримом. Может проксировать любой протокол поверх TCP/UDP (БД, MQTT, игровые серверы, DNS, свой бинарный протокол) — именно потому, что не пытается ничего в этих байтах разбирать. Никакой маршрутизации по содержимому, переписывания заголовков или кэширования тут нет — только "принял с одной стороны → переслал на другую".
>
> Промежуточный случай: `ssl_preread` в `stream{}` умеет подсмотреть SNI из TLS ClientHello (не расшифровывая трафик) для маршрутизации HTTPS по домену ещё до терминации TLS — но это не "понимание протокола", а точечный разбор одного заранее известного формата.
>
> **Чего nginx не умеет вообще:** он не роутер и не L2/L3-устройство — работает с TCP/UDP-сокетами и выше, не с сырыми IP/Ethernet-пакетами.

>[!question]- как nginx понимает, на каком протоколе прилетел пакет — ведь в `stream{}` вообще нет заголовков?
> Nginx **не распознаёт** протокол по содержимому пакета в рантайме — это решается заранее, на этапе конфигурации, по тому, на какой порт/листенер пришло соединение.
>
> - В `http{}`: `server { listen 443 ssl http2; ... }` — nginx слушает порт 443 и **безусловно** считает, что всё, что туда прилетело, это HTTP, и пытается распарсить как HTTP-запрос.
> - В `stream{}`: `server { listen 6432; proxy_pass db; }` — nginx слушает другой порт и просто перекачивает байты как есть, не пытаясь их парсить.
>
> Это два разных модуля (`ngx_http_module` и `ngx_stream_module`), каждый со своим набором директив. Администратор сам решает на этапе написания конфига, что означает каждый порт. При старте nginx читает весь конфиг, видит все `listen`-директивы в обоих блоках и открывает соответствующие сокеты, каждый заранее привязанный к своему обработчику.
>
> **Что будет при мусоре не того протокола на HTTP-порт:** nginx не пытается угадать другой протокол — просто не сможет распарсить как HTTP и оборвёт соединение/отдаст `400 Bad Request`. Никакого fallback-распознавания не существует.
>
> В `stream{}` заголовков действительно нет, и именно поэтому там нет и не может быть логики распознавания — nginx принципиально не заглядывает внутрь байтов (кроме узкого исключения `ssl_preread`, о котором выше — но это тоже не автоопределение протокола, а заранее известный, явно включённый разбор одного конкретного формата).
>
> Итог: протокол определяется не пакетом, а **портом/листенером**, прописанным в конфиге. Одна nginx-инсталляция может одновременно слушать порт 443 как HTTP и порт 6432 как сырой TCP — это два независимых слушателя без какого-либо взаимного "распознавания".

>[!question]- Как именно nginx проксирует WebSocket handshake — кто шлёт 101, сколько TCP-соединений
> nginx в режиме `proxy_pass` (HTTP-уровень, блок `http{}`) — не прозрачный провод, а полноценный HTTP-клиент/сервер на границе. Реальная картина:
> ```
> Клиент ⇄(TCP-соединение #1)⇄ nginx ⇄(TCP-соединение #2)⇄ бэкенд (Tomcat)
> ```
> Это **два независимых** TCP-соединения, они никогда не сливаются в одно сквозное — в отличие от прямого подключения клиента к Tomcat (см. [[Spring Mvc]], калаут про handshake).
>
> 1. Клиент шлёт `GET` с `Upgrade: websocket` по соединению #1.
> 2. nginx **не пересылает эти байты как есть** — он разбирает исходный запрос и строит **новый** HTTP-запрос к апстриму по соединению #2 (поэтому и нужны явные `proxy_set_header`, см. следующий калаут).
> 3. `101 Switching Protocols` генерирует **сам бэкенд** (Tomcat/Spring), а не nginx. nginx лишь получает этот ответ по соединению #2 и пересылает клиенту по соединению #1.
> 4. Как только nginx видит `101` — он переключает **оба** своих соединения в режим "тупого" байтового туннеля: дальше просто перекачивает сырые байты в обе стороны, больше не парся HTTP/WS-фреймы вообще.
>
> То есть nginx и Tomcat независимо переключают интерпретацию каждый своего сокета с HTTP на WebSocket-фреймы, синхронизированные одним событием (успешный handshake), но это два отдельных переключения, а не одно общее.

>[!question]- Почему для WS нужно вручную дублировать `Upgrade`/`Connection` в `proxy_set_header`, а `Content-Type`/`Cookie` nginx пробрасывает сам
> В HTTP-спецификации есть небольшой явный список **hop-by-hop** заголовков: `Connection`, `Upgrade`, `Keep-Alive`, `TE`, `Trailer`, `Transfer-Encoding`, `Proxy-Authenticate`, `Proxy-Authorization`. Они описывают свойства **конкретного соединения** (client↔nginx), а не самого запроса/ресурса — поэтому любой корректный прокси обязан **не** пробрасывать их дальше автоматически.
>
> Всё остальное — `Content-Type`, `Cookie`, `Authorization`, `Accept`, тело JSON, кастомные заголовки — это **end-to-end** заголовки, описывающие сам запрос, и nginx пересылает их на апстрим автоматически, без всякой настройки.
>
> Поэтому в конфиге:
> - `proxy_set_header Upgrade $http_upgrade;` / `proxy_set_header Connection 'upgrade';` — явно повторяем hop-by-hop заголовки уже для второго соединения (nginx→бэкенд), иначе бэкенд не поймёт, что нужен апгрейд, и просто ответит как на обычный HTTP-запрос.
> - `proxy_set_header Host $host;` — формально не hop-by-hop, но у nginx своё дефолтное поведение: без переопределения он подставит в исходящий запрос адрес самого апстрима (`127.0.0.1:6300`) вместо исходного домена клиента.
> - `proxy_set_header X-Real-IP $remote_addr;` — восстанавливает IP клиента для бэкенда (подробнее про `$remote_addr` — см. калаут ниже).
>
> Простое правило: **"заголовки про соединение" (`Upgrade`/`Connection`) нужно пересказывать вручную, "заголовки про содержимое запроса" пересказываются автоматически.**
