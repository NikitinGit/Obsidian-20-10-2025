...
---
1. [ ] агрегат отличается от сущности тем что агрегат может представлять взаимосвязанную систему сущностей, которые логически не могут существовать изолировано друг от друга ?
2. [ ] что такое bounded context - чем определяются границы и как это связано с микросервисами
3. [ ] пример domain event в спринг 
4. [ ] что хорошо в правиле "бизнес логика находится в домене / сущности" а что плохо. 
5. [x] что входит в доменый слой 
6. [x] фабричный метод 
7. [ ] пример разбиения домена на поддомены 
## Основные концепции
...
> Была предложена Эриком Эвансом в ==**2003 году**==. Год создания ознаменован выходом его основополагающей книги _«Domain-Driven Design: Tackling Complexity in the Heart of Software»_,
> **Зачем:**
...
>[!question]- Что входит и НЕ входит в домен?
>1.  **Агрегаты (Aggregates)** - **Entity** со своим id , Value Objects без id
>2. **Доменные события** (например, `EventStarted`, `FighterRegistered`)
>3. **Интерфейчы репозиториев** без технической реализации. Сама реализация (код, который пишет SQL-запросы) лежит в слое инфраструктуры.
>4. **Доменные сервисы** - классы в которых находится бизнес логика не помещаюшаяся в одну сушность.
>5. **Исключения домена** (Domen Exceptions) - (например, `InsufficientFundsException`, `TournamentFullException`). Мы не используем здесь технические ошибки вроде `SQLException`.
>6. **Фабрики (Factories)** - если создание агрегата вырастает в огоромный кусок кода - его переносят в отдельлную фабрику
>
> **Что не входит** - 
> 1. Контроллеры -(это слой Web/API). 
> 2. DTO - (это слой представления или обмена данными).
> 3. **Код работы с БД** (SQL, JPA-репозитории, подключения). 
> 4. ЛОгика отправки смс / почта 
> 5. **Мапперы / Assemblers** (это прикладной слой).
...
> 
>**Что входит:**
>1. **Aggregate Root (Корень)** — главная Сущность (например, класс `Event`). Это «лицо» агрегата, через которое идет все общение.
>2. сущности - @Entities
>3. Value Object
>4. бизнес логика
>
>**Что НЕ входит:**
>1. **Repository (Репозиторий)** — это инструмент инфраструктуры. Агрегат не знает, как он сохраняется в базу. Наоборот, репозиторий работает с агрегатом.
>2. **Domain Service (Доменный сервис)** — это внешний «помощник». Если логика требует участия двух разных агрегатов (например, перевод денег с одного `Account` на другой), она живет в сервисе, а не внутри агрегата.
>3. **Application Service (Прикладной сервис)** — это «координатор». Он достает агрегат из базы и вызывает его методы.
>4. **Другие Агрегаты** — как мы обсуждали, агрегат может содержать только **ID** другого агрегата, но не сам объект.
>5. **DTO** — это объекты для передачи данных во внешний мир, они живут на уровне API или Assembler.
>
> **Правила агрегатов:**
> 6. Внешний код обращается только к Root
> 7. Инварианты (бизнес-правила) поддерживаются на уровне всего агрегата
> 8. Агрегат сохраняется/загружается целиком
> 9. Ссылки между агрегатами — только через ID
>
> **Пример:**
> ```
> Order (Root)
