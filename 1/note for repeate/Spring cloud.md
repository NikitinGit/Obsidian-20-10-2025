...
2. [ ] чем отличается @Bean  созданный внутри @Configuration класса от просто внутри класса ?- ДОПИШИ и посмотри в коде 
3. [ ] возможно ли прописать креды подключения к БД не через апликатин.проперис а через @Configuration  , если да , то напиши пример 
4. [ ] hikary poll это 
5. [ ] как еврика или конфиг или что то еще может определить насколько сильно забит инстанс запросами и как сильно занят вычислениями ? что в спринг за это отвечает ?
6. [ ] напиши несколько  модулей в мавен в микросервисах  - все микросервисы в одном монорепо 
7. [ ] ДРУГИЕ БАЛАНСИРОВЩИКИ - API Gateway balancing , DNS Load Balancing ,  **Service Mesh balancing** 
8. [ ] в каких бизнес задачах обычно нужна балансировка ? 
9. [ ] ЗАДАЧА написать микросервисную систему в которой - 
10. [ ] при создание нового инстанса он подхватывает нужные ему настройки из конфига 
11. [ ] работает балансировка 
12. [ ] как балансировщик может отслеживать нагрузку на инстанс 
13. [ ] как откатить инстанс назад и как при этом должен работать пайплайн 
14. [ ] как произвести транзакции в БД на 2  инстанцах 
