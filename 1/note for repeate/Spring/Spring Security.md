...
2. [ ] Что такое HttpOnly куки
3. [ ] какие бывают куки кроме HttpOnly 
4. [ ] как спринг отправляет токен пользователю после захода на сайт 
5. [ ] как фронт принимает и отправляет этот токен, как он его хранит и может ли управлять его хранением
6. [ ] какие алгоритмы могут быть у jwt хедера
7. [ ] сколько в хедере может хранится данных - определяется браузером и бэкендом
8. [ ] в ServletRequest/doFilter находится только хедер ? как получить боди
9. [ ] GenericFilterBean единственный способ перехватить запрос ?
10. [ ] напиши приложение спринг секурити , в котором есть ендпоинт, конфиг, класс обработки  jwt , вспомни что такое crsf, cors 
11. [ ] напиши фронт для посылки запросов с хедором jwt (есть ли в хедере мими типы)
12. [ ] пример с сессиией 
13. [ ]  @AfterReturning(pointcut = "@within(checkEventOwnership)", returning = "result") 
14. [ ] сквозная функциональность - общая для всех методов ?
15. [x] у нас в проекте секрет токена (сигнатура jwt) хранится в коде - это называется не соль а перец? - нет, это HMAC секрет
