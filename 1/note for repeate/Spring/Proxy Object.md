...
>в конце процесса создания бина перед помещением его ApplicationCotext  и отдаст вашему коду. Через newProxyInstance после сканирования проекта ( всех бинов)  - не при расширении интерфейса extends JpaRepository<Event, Integer>
...
>     - **Сразу же внедряет** в него зависимости (через `@Autowired` или конструктор), следуя паттерну Синглтон (ищет уже готовые бины в своем хранилище).
...
> - **Самый финал:** Специальный обработчик (`BeanPostProcessor`) проверяет, нужны ли бину транзакции или кэш. Если нужны, он вызывает `Proxy.newProxyInstance()` для JDK Dynamic Proxy.
> - **Важно:** Полученный прокси-объект **заменяет** собой оригинальный сырой объект в хранилище Spring (IoC-контейнере). Все последующие классы, которые будут запрашивать этот бин, получат уже этот прокси.
...
>[!question]- Имеет ли значение порядок аннотаций?
>**Нет, порядок `@Autowired @Lazy` или `@Lazy @Autowired` не имеет абсолютно никакого значения.**
>Для Java-компилятора и для рефлексии (Reflection API), которую использует Spring, аннотации над полем или конструктором — это просто «мешки с метаданными». Когда Spring сканирует класс, он запрашивает список всех аннотаций у поля скопом (через метод `field.getAnnotations()`). Spring проверяет _наличие_ обеих аннотаций, а не то, какая из них написана левее или выше.
...
>[!question]- когда выозникает N + ! в lazy-полях 
> 
> ```java
> List<EventBidFighter> bids = eventBidFighterRepository.getFightersFromBids(17);
> String name = bid.getEvent().getEventId().toString(); /// не возникает только тут
> name = bid.getEvent().getNameEvent(); // при обращении к другим полям возникает 
> ```
