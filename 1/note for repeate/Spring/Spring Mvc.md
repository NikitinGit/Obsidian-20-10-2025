...
7. [ ] как настраивать пул потоков в application.properties - как определить цифры 
8. [ ] "Он отвязывает обработку запроса от конкретного потока: поток, принявший запрос, сразу возвращается в пул, а запись в открытый response происходит позже, из другого места, когда появляются новые данные." - где он хранится тогда ?
9. [ ] Важное замечание о Виртуальных потоках (Java 21+) и прочти статью https://medium.com/@gaddamnaveen192/we-replaced-500-tomcat-threads-with-virtual-threads-the-results-shocked-us-12cd9afe4a15 
10. [ ] **Асинхронные методы (`@Async`)**:
11. [ ] Spring webflux
# Spring MVC
