...
12. [ ] @EqualsAndHashCode.Exclude @ToString.Exclude - проверь в коде все проблемы с бесконечной рекурсией, не верным сравнением объектов и проблемой N+1
13. [x] PreAutorize / Auth 
14. [ ] JPA / Hibernate VS Active Record 
15. [ ] Active Record точно нарушает SRP ? 
16. [ ] jpql к чему относится в  хайбернате
17. [ ] в mysql 5.7 нужно сохранить длительность боя в часах минутах секундах, какой должен быть тип этого поля  - в секундах, достаточно integer , на java исползуется Duration  - напиши памятку 
18. [ ] обязательно ли указывать в hibernate timezone и как принято хранить время с чаовым поясом в Spring (например фронт отправляет время и часовой пояс какого то события, а так же время с датой его  отправки ) - желательно - напиши памятку 
19. [x] EntityManager - зачем нужен detach , когда исползуется и другие методы 
20. [x] динамический прокси-объект - существует ли статический - да (самописный) , допиши 
21. [ ] интерсептор и мидлвеар - допиши
22. [x] детальнее про фабричный метод патерн - почему @Bean это он
23. [x] Проблемы с Hibernate прокси:- как хайбернейт создает проки объект в которм equals | hashcode не работает - 
24. [x] Безопасно ли при выборке делать сравнение с объектом сущостью или лучше с id - List\<JudgeScore> findAllByBattleAndJudge(Battle battle, Judge judge); \-  когда могут возникнуть проблеммы ?
25. [ ]  Почему в JudgeScoreRepository можно делать выборку других сущностей не относящихся к этому репозиторию и правильно ли так делать 
26. [ ] Прочитай https://docs.spring.io/spring-framework/reference/web.html и сравни со своим проектом
27. [x] Попробуй вариант Решение 2: QueryDSL (IDE видит usage!) в клоде - который должен показывать usage  
28. [ ] как кэшировать гет запросы 
29. [ ] Domain-Driven Design (Доменно-ориентированное проектирование)  
30. [ ] Всегда ли есть смысл делать связь на уровне ентити а не только на уровне БД ?
31. [ ] какие аннотации надо знать на собеседовании 
32. [ ] @Profile
33. [ ] @ConditionProperty
34. [ ] КЭШ первого и второго уровня
35. [ ] возможно ли подлкючиться к бд без application.properties
36. [ ] как запустить не ddl миграции в спринг - посмотри в MigrationService.java 
37. [ ] встроенный серверы в Spring boot - Tomcat, Jetty 
38. [ ] отличие @GetMapping("/hello") от @RequestMapping(value = "/hello", method = RequestMethod.GET)
39. [ ] AOP в Spring 
40. [ ] Как создается изолирванный контекст 
41. [ ] hikary poll 
42. [ ] Подключения к БД:  credentials, pool settings
43. [ ]  благодаря поддержке транзакций, интеграции с JMS, Kafka и другими сообщениями, Spring активно используется для создания банковских систем, страхования, складских систем и документооборота.
44. [x] елк стек, актуатор , графана 
