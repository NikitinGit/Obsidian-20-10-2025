...
2. [ ] где в проекте исползьуется JPA а где Hibernate 
3. [ ] проверь **` LEFT JOIN FETCH`** по числу подзапросов - сравни с LEFT JOIN
4. [ ] SimpleBroker durability - чем отличается от rabbit mq
5. [ ] SpringSecurity Autentification - как добавить в контекст или в HandlerInterxeptor , как вебсокеты и в рест доавляются jwt,  что такое мидлвеер 
6. [ ] [[SMS]]
7. [ ] [[MAIL]]
8. [ ] [[Date and Time]]
9. [ ] кеш в ответе , где хранится, как создается зависимость , как добавить в код 
10. [ ] транзакции оплаты юкассы и сохранение фоток при создании соревнования 
11. [ ] applicatioin.properties -  настройки контейнер сервлетов - на что влияет число подключений, запросов ?
12. [ ] в скл запросах можно менять COMMENT - AFTER местами ?
13. [ ] как и где используется АОП - тесты, авторизация (можно ли ставить аннтоцию проверки прав надо классом сервиса)
14. [ ] Как работает jwt в спринг секурити 
15. [ ] мавен - все параметры
16. [ ] docker как устроены все элементы
17. [ ] [[Ansible]]
...
4. [ ] докер файл - зачем нужен ньюрелик - логи и метрики
5. [ ] докер файл ансибл - что такое роли и зачем 
6. [ ] как работает nginx и где прописан его конфиг 

# Монитроинг
1. [ ] графана как работает (получает данные из прометеус ?)
2. [ ] есть ли еластик серч (да, кибана получает из еластик серча данные или из лог стеша? - из еластика серча , лог стеш нужен чтобы обработать данные от приложения и передать их в еластик , если **не нужно обрабатывать данные, то можно logStash заменить на FileBeat** - Например, приложение пишет уже готовый JSON)
3. [ ] кебана это


# Test
1. [ ] как написть тесты экспорта в ворд и эксель (например настройки отступов страницы проверить)
