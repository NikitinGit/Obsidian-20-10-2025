...
1. [x] если хеширование основано на потере данных, то за счет чего достигается уникальность кадого хеша, как предотвращаются коллизии ? (см. «Уникальность хеша не гарантируется», «Что внутри SHA-256»)
2. [x] почему используется guava для хеширования  HMAC secret - это такой стандарт? (см. «Чем считать HMAC: Guava vs javax.crypto.Mac vs JWT-либы»)
3. [ ] Правильно ли я понимаю что в этом проекте испольщзуется кастомный jwt ? Если так, то что может предложить Spring Security для использования jwt
4. [ ] Замена Guava на `Mac` НЕ чинит главную проблему — сравнение `signatureExpected.equals(signatureCurrent)` (строка 213) не constant-time → timing side-channel. Правильно: `MessageDigest.isEqual(expectedBytes, actualBytes)`. Это отдельно от выбора библиотеки.   КАК ПРОВЕРИТЬ ПРИ ДЕБАГЕ ?
5. [x]  все ли jwt хранят данные в Base64Url - по спецификации Да
6. [ ] что такое Base64 - как происходит кодирование
7. [ ] https://www.google.com/search?udm=50&sourceid=chrome&ccb=1&q=%D1%81%D0%BF%D0%B5%D1%86%D0%B8%D1%84%D0%B8%D0%BA%D0%B0%D1%86%D0%B8%D1%8F+JWT&gs_lcrp=EgZjaHJvbWUyBggAEEUYOTIICAEQABgWGB4yCAgCEAAYFhgeMgoIAxAAGAoYFhgeMggIBBAAGBYYHjIICAUQABgWGB4yCAgGEAAYFhgeMgcIBxAAGO8FMgcICBAAGO8FMgoICRAAGIAEGKIE0gEIMTQxNWowajeoAgCwAgA&ie=UTF-8&fbs=ABfTbFXwfYg-AsVmc75aAkBispm6k7RIMh52cWHUntfypj3niv0vpWpy8F5Hmr7ocaLGY8yxo0o9z-YF4eXeo5Zl7j7dDjRDEr_liE8hrc7M-RfSwuOJWYhl5sgRH_flsRnOYxJ6IWRBDNcvNL2mMoqSlv7LRGFScMpD42IGMdpjhQp8E6FvSMh6vHhQcd1R-FQTFtzle7bi9ZTjQAuReQ7p5jAms0NivYfAoYNVQLKer_HtggnC-zLUGwBxa1sYpa-KoDdU9ebB26gz1z9c3iv46ijq1SvRDQ&aep=10&ntc=1&mstk=AUtExfAmRbqgyhxUReClBU66uaAegtUSlthTOwDZl4RvL_YTDs0pxtNxjisclBIOKlLE7Vk0U_QDy63talai76-lTKk0fmcAGqSbuG2kQCP48pYp9O-CKRXmc_tz0sEpEf2voVnjJiAn4JjFGBbfA7DpDJ708ZptptdNwNc&aioh=3&csuir=1&cs=1&hl=en-US&biw=2048&bih=1056.800048828125&mtid=HeY8av6BBrWg5NoPlY-d0QQ
8. [x] почему в хедер и нагрузка складываются - это такой способ - спецификация  JWT

>[!question]- Структура JWT (header / payload / signature)
>Токен = `header.payload.signature`, каждая часть в **base64url**, разделены точками. Base64url единственный формат кодировки.
>```
...
>[!question]- Как принято подписывать токен
>1. **Кодирование**: Заголовок (Header) и Полезная нагрузка (Payload) независимо друг от друга переводятся в формат **Base64url**.
>2. **Соединение (Конкатенация)**: Полученные строки соединяются в одну через **точку** (`.`).
>3. **Хеширование с ключом**: К этой итоговой строке применяется выбранный криптографический алгоритм (например, HMAC-SHA256) с использованием вашего секретного ключа.

>[!question]- Типы JWT
>- **JWS (JSON Web Signature):** Подписанный токен. Полезная нагрузка защищена от изменений, но данные остаются открытыми для чтения
>- (они просто закодированы в Base64url). [[1](https://datatracker.ietf.org/doc/html/rfc7519), [2](https://jwt.io/introduction)]
>- **JWE (JSON Web Encryption):** Зашифрованный токен. Обеспечивает не только целостность, но и конфиденциальность содержимого, скрывая данные от сторонних наблюдателей. [[1](https://datatracker.ietf.org/doc/html/rfc7519), [2](https://jwt.io/introduction)]
...
>[!question]- Уникальность хеша НЕ гарантируется — что мешает юзеру 17 получить права юзера 7
>**Коллизии есть всегда** (Дирихле): входов ∞, выходов конечно (2²⁵⁶). У SHA-256 коллизии существуют, просто их никто не находил. Фокус не в том, чтобы их не было, а в том, что их **вычислительно невозможно найти**.
>Заблуждение, что страшно: «хеш ИДЕНТИФИЦИРУЕТ юзера, сервер сверяет хеш payload'а с правами». Это НЕ так — защиту даёт не уникальность хеша, а **секрет внутри HMAC**.
>Сценарий: юзер 17 хочет права юзера 7, меняет payload на `{"id":7}`:
>1. payload изменился → сервер пересчитывает HMAC от НОВОГО payload'а → подпись другая → `expected.equals(current)=false` → отклонён;
>2. чтобы подпись подошла, нужен правильный HMAC для `{"id":7}`, а для этого нужен **secret** (`Hashing.hmacSha256(secret...)`) — его у атакующего нет.
>Даже если бы коллизия SHA-256 нашлась — без секрета подпись не подобрать (HMAC замешивает секрет внутрь). Защита = КЛЮЧ, не уникальность.
>Где `mod 10` реально сломал бы JWT: подпись = `payload mod 10` → выход крошечный (10 вариантов) и предсказуемый → перебираешь 10 значений за секунду, секрет вообще не нужен. SHA-256/HMAC убивает оба вектора: выход 2²⁵⁶ (перебор невозможен) + секрет обязателен.

>[!question]- Что внутри SHA-256 — лавинный эффект на 2 раундах
>Вход отличается на 1 символ, выход — общего почти нет:
>```
>{"id":7}  → a3c90e3b 7448d23d ...
>{"id":17} → ba5952a0 8cb17757 ...   (разное с первого байта)
>```
>Это **лавинный эффект (avalanche)**. Внутри SHA-256 держит **8 переменных по 32 бита** (`a..h`), сообщение режется на 32-битные `W[i]`, и за **64 раунда** их перемешивает. Один раунд (упрощённо):
>```
>T1 = h + Σ1(e) + Ch(e,f,g) + K[i] + W[i]
>T2 = Σ0(a) + Maj(a,b,c)
>h=g g=f f=e e=d+T1 d=c c=b b=a a=T1+T2
>```
>Три ингредиента хаоса:
>- **Σ (sigma)** — циклич. сдвиги + XOR (`Σ1(e)=rotr(e,6)⊕rotr(e,11)⊕rotr(e,25)`): бит «расползается» по слову;
>- **Ch, Maj** — побитовые «если/большинство»: мешают переменные между собой;
>- **`+` mod 2³²** — перенос тянется из младших бит в старшие (нелинейность).
>Игрушка (8-битные слова, `rot`=сдвиг влево на 3, раунд `a=rot(a)⊕a⊕W`), входы отличаются 1 битом в W:
>```
>Раунд1: A: 00001000⊕00000001⊕00000111 = 00001110
>        B: 00001000⊕00000001⊕00010111 = 00011110   (1 бит разницы держится)
>Раунд2: rot разносит этот бит в др. позицию, ⊕ подмешивает → разница уже в неск. битах
>```
>В реале раундов 64, переменных 8, каждая втекает в следующую → после ~3-4 раундов любой входной бит влияет на ВСЕ 256 выходных. Отсюда `a3c9…` vs `ba59…`. Это же даёт и необратимость: отмотать 64 перемешивания назад нельзя, подобрать коллизию нельзя — каждый бит влияет на всё, остаётся слепой перебор ~2¹²⁸.

>[!question]- Какая наука стоит за хешированием
>| Уровень | Название | Чем занимается |
>|---|---|---|
>| широкая наука | **Математика** | теория чисел, алгебра, вероятность — фундамент |
>| раздел | **Криптология** | наука о шифрах, делится надвое ↓ |
>| ↳ создание | **Криптография** | проектирует шифры/хеши/подписи (SHA-256, HMAC, RSA) |
>| ↳ взлом | **Криптоанализ** | ищет слабости/коллизии (он «убил» MD5 и SHA-1) |
>
>Хеши конкретно — раздел криптографии «**криптографические хеш-функции**». Смежное:
>- **Теория сложности (CS)** — объясняет, ПОЧЕМУ перебор 2¹²⁸ «невозможен» (one-way functions, рядом P vs NP);
>- **Теория информации (Шеннон)** — энтропия + принципы **confusion & diffusion**: ровно то, что делают `Ch/Maj` (confusion) и `Σ`-сдвиги (diffusion);
>- **Теория вероятностей** — birthday paradox: коллизию найти за ~2ⁿ/², а не 2ⁿ;
>- **Дискретная математика** — арифметика mod 2³², битовые операции, конечные поля.
>Стандартизирует на практике **NIST** (SHA-2/SHA-3, конкурсы AES/SHA-3).
...
>[!question]- Чем считать HMAC: Guava vs javax.crypto.Mac vs JWT-либы
>Guava для HMAC — **НЕ стандарт**, выбрана по принципу «уже в проекте» (нужна для коллекций/утилит, а `Hashing.hmacSha256` даёт fluent-API в одну строку). Под капотом Guava всё равно зовёт **JDK-шный `javax.crypto.Mac`** — вот он и есть настоящий «стандарт» (JCA = Java Cryptography Architecture, встроена в JDK).
>Альтернативы для `generateBase64UrlEncodedSignature`:
>1. **Голый JDK `javax.crypto.Mac`** — без сторонних зависимостей, канонический способ:
>```java
>Mac mac = Mac.getInstance("HmacSHA256");
>mac.init(new SecretKeySpec(secret.getBytes(UTF_8), "HmacSHA256"));
>byte[] hash = mac.doFinal(message.getBytes(UTF_8));
>return encodeBase64Url(hash);
>```
>2. **Apache Commons Codec** — `new HmacUtils(HmacAlgorithms.HMAC_SHA_256, key).hmac(message)`.
>3. **JWT-библиотека целиком** (подпись руками не пишешь вообще): jjwt (`Jwts.builder()…signWith(key)`), java-jwt (`Algorithm.HMAC256(secret)`), nimbus-jose-jwt (`MACSigner`). Сами делают header/payload/sig + проверку exp + защиту от `alg:none`.
>
>| Вариант | Когда |
>|---|---|
>| `javax.crypto.Mac` | сам контролируешь JWT, но без лишних зависимостей |
>| Guava (текущее) | Guava и так в проекте; выбор случайный, не обоснованный |
>| jjwt / nimbus | **для прода** — закрывают alg confusion, парсинг, exp |
>
>⚠️ Замена Guava на `Mac` НЕ чинит главную проблему — сравнение `signatureExpected.equals(signatureCurrent)` (строка 213) не constant-time → timing side-channel. Правильно: `MessageDigest.isEqual(expectedBytes, actualBytes)`. Это отдельно от выбора библиотеки.
