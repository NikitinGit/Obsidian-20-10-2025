...
Уведомления судьям/организаторам (старт боя, результат, добавлен/удалён бой следующего раунда и т.д.) идут по WebSocket поверх протокола STOMP.

---
>[!question]- TCP соединение 
>**TCP** соединение определяется четвёркой чисел - IP клиента, порт клиента, IP сервера, порт сервера. Только вся четвёрка вместе уникальна.

>[!question]- зачем нужен порт клиента в TCP соединение 
>Потому что один и тот же IP могут использовать много разных соединений:
>- Ты открыл две вкладки в браузере — обе идут с одного IP твоего компьютера. Различаются они только портом: одна вкладка с порта 54321, другая с 54322.
>- Целый офис или дом за одним роутером (NAT) выходит в интернет под одним публичным IP. Сервер видит десятки клиентов с одинаковым IP, и единственное, что их различает, — порт.

>[!question]- где хранится порт и IP клиента на сервере
>Хранит это не приложение вручную, а ядро операционной системы. При установке соединения ядро создаёт **сокет** — объект в памяти, внутри которого лежит вся эта четвёрка (IP и порты обеих сторон).
>Приложение (твой сервер) получает просто "ручку" на этот сокет. 
>"Ручка" — это перевод английского слова **handle**. Более технически точный термин в мире Unix/Linux — **файловый дескриптор** (file descriptor, сокращённо fd). И вот тут ключевой момент: приложение хранит **не указатель на память ядра**, а просто **маленькое целое число**.
>Когда ядро создаёт сокет, вся реальная структура данных (с той самой четвёркой IP+порты, буферами, состоянием соединения) живёт в памяти ядра — в адресном пространстве, куда приложение вообще не имеет прямого доступа. Приложение не может взять указатель и залезть туда напрямую. Вместо этого ядро говорит приложению: "вот тебе номер 7, когда захочешь что-то сделать с этим сокетом — называй мне этот номер". 

>[!question]- что может порвать соединение 
>- Само ядро **по неактивности** соединение **не закрывает** — в TCP такого механизма нет
>- Ядро может проверять живость (keepalive) и закрывать мёртвые соединения, но **только если приложение это включило** (`SO_KEEPALIVE`), и по умолчанию это выключено, а тайминги огромные (2 часа).
>- Ядро закроет соединение **по событиям** (неудачная доставка данных, приход RST) — но это не про таймаут молчания, а про реальную недоставку или сигнал с той стороны.
>- А вот **промежуточные NAT/файрволы** закроют молчащее соединение без спроса (как при статическом, так и при динамическом ip адресе— и это то, от чего heartbeats в STOMP и спасают.
## Архитектура
...
>[!question]- cookie vs обычный заголовок — в чём разница (и почему WS авторизуется кукой)
> Cookie физически **передаётся внутри заголовка**, так что технически это частный случай заголовка. Для куки зарезервированы два заголовка:
> - клиент → сервер: `Cookie:` (в нём разом все куки домена);
> - сервер → клиент: `Set-Cookie:` (сервер говорит «сохрани такую куку»).
> ```http
> GET /api/events HTTP/1.1
> Authorization: Bearer eyJ...          ← обычный заголовок, JS выставил ВРУЧНУЮ
> Cookie: strikerstat_token=eyJ...      ← заголовок Cookie, браузер подставил САМ
> ```
> **Но по поведению это разные вещи:**
> | | Обычный заголовок (`Authorization`) | Cookie |
> |---|---|---|
> | Кто ставит | код вручную на каждый запрос (`fetch({headers})`, `curl -H`) | сервер один раз через `Set-Cookie`, дальше браузер |
> | Хранение | не хранится, живёт один запрос | браузер хранит по домену (атрибуты: срок, `Domain`, `Path`, `Secure`, `HttpOnly`, `SameSite`) |
> | Автоотправка | нет — не приложишь сам, не уедет | **да** — браузер сам цепляет ко всем запросам того же origin |
> | Доступ из JS | JS видит/ставит | `HttpOnly`-куку JS **не читает** (защита от XSS), но браузер её всё равно шлёт |
> Аналогия: обычный заголовок пишешь от руки на каждом конверте; cookie — штамп, который браузер сам ставит на все конверты в этот адрес и помнит между визитами.
> **Почему это важно для WS:** браузерный `new WebSocket()` не даёт выставить заголовки → `Authorization: Bearer` для WS невозможен. А **cookie на хендшейк едет автоматически** (тот же origin). Поэтому WS-авторизацию строят на куке, а не на Bearer-заголовке.

>[!question]- как авторизуется WS в StrikerStat (JWT в cookie, проверка на хендшейке)
> Токен — JWT в **cookie `strikerstat_token`** (`AuthService.AUTH_TOKEN_COOKIE_NAME`), один и тот же для REST и WS.
> - **REST** — `TokenAuthenticationFilter`: сначала cookie `strikerstat_token`, если нет — фолбэк на `Authorization: Bearer`. Проверка на **каждом** запросе.
> - **WS `/ws`** — `WebSocketHandshakeInterceptor.beforeHandshake`: читает cookie из HTTP-запроса хендшейка → `parseJwtToken` → `parseAuth` → `isAuthenticated`. Валиден → `attributes.put("auth", auth); return true`; иначе `return false` → **403, соединение не встаёт**.
> **Нюансы:**
> 1. WS авторизуется **только кукой** (Bearer-фолбэка нет — заголовок на браузерный WS не поставить).
> 2. Проверка **один раз на хендшейке**, не на каждое STOMP-сообщение: после `Upgrade` TCP держится открытым, HTTP-запросов больше нет → куку переслать некуда, auth привязан к сессии.
> 3. Из терминала не подключиться: `wscat -c ws://localhost:6300/ws` → нет cookie → отказ (это и есть цель интерсептора).
> 4. **`/ws-public` — без интерсептора вообще** (`WebSocketConfig`: только `setAllowedOriginPatterns("*")`). Он публичный, т.к. оверлей/OBS смотрит аноним без JWT-cookie. Поэтому трансляция на `/ws-public`, а судейские уведомления (залогиненные) — на `/ws`.
...
>[!question]- ВАЖНО: серверный nginx-конфиг генерится Ansible — правки на сервере ВРЕМЕННЫ (2026-07-03)
> Файл `/etc/nginx/sites-available/front.conf` на сервере **не редактируется руками** — он рендерится Ansible'ом из шаблона `front-nginx.conf.j2` (репо `ansible-playbooks`, роль `nginx-config`):
> ```yaml
> template: { src: front-nginx.conf.j2, dest: /etc/nginx/sites-available/front.conf }  # → nginx -t → reload
> ```
> **Как обожглись:** правили `/ws-public` и `allow` прямо на сервере — работало, пока выгрузки Ansible были **сломаны** (протух git-токен, деплой не шёл). Синьор пересоздал токен → Ansible прогнался → перезаписал конфиг из шаблона (где наших правок нет) → всё **слетело**.
> **Мораль — менять ШАБЛОН, не сервер:**
> - `location /ws-public`, `proxy_read_timeout`, прочие location'ы → в `front-nginx.conf.j2`;
> - разрешённые IP (цикл `for ip in testing_allowed_ips` в шаблоне) → в переменную **`testing_allowed_ips`** (она в inventory/host_vars, приватно — НЕ в ansible-репе; спросить того, кто ведёт inventory);
> - на проде авторизации нет вообще (блок под `if server_env != 'prod'`), так что `auth_basic off; allow all;` в `/ws-public` там безвредны.
> **Раскатка:** PR в ansible-репу → плейбук `deploy-front.yml` (дёргает роль `nginx-config`). Пайплайн ведёт синьор — согласовывать, не пушить молча.
> **Проверка «что реально на сервере»:** `nginx -T` — но это лишь СЛЕПОК последнего рендера; источник правды — шаблон. Ручной `nginx -s reload` тоже временный: переживёт до следующего прогона Ansible.
> **Про `allow all` в `/ws-public`:** открывает публично **только этот сокет** (IP тестировщика для WS не нужен). Страница оверлея / `/_nuxt` / `/api` остаются под server-level `satisfy any` → тестировщику для работы в браузере всё равно нужен **пароль** (или IP); для OBS — страница по URL с кредами, сокет по `/ws-public`.
